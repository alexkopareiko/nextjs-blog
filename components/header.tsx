...
class Header extends React.Component<MyProps, MyState> {
  ...
  render() {
    ...
    const { userFirstName, userLastName, userImg } = this.props.identity;
    const { isOpenSandwich, isOpenProfile } = this.state;
    const NOT_AUTHORIZED = this.props.identity.userId === -1;
      ...
      <header className="bg-gray-900 sm:flex sm:items-center sm:justify-between xl:bg-white">
        <div className="flex justify-between py-4 px-3 xl:w-72 xl:bg-gray-900 xl:justify-center xl:py-5">
          ...
          <div className="flex sm:hidden">
            <button type="button" className="px-2" onClick={() => { this.setState({ isOpenSandwich: !isOpenSandwich }) }}>
        ...
        <nav className={`${!isOpenSandwich ? 'hidden' : 'block'} sm:flex sm:items-center xl:flex-1 xl:justify-between`}>
          ...
          <div className="sm:flex sm:items-center">
            ...
            <div className="relative px-5 py-5 sm:py-0">
              <div onClick={() => { this.setState({ isOpenProfile: !isOpenProfile }) }} className="flex items-center">
              ...
              <button type="button" onClick={() => { this.setState({ isOpenProfile: !isOpenProfile }) }} className={`${!isOpenProfile ? 'sm:hidden' : 'sm:block'} sm:fixed sm:opacity-1 sm:inset-0 sm:w-full sm:h-full sm:z-40`}></button>
...
const mapDispatchToProps = (dispatch) => {
  const action = identityEntity.getOneAction('sagaLogout');
  return {
